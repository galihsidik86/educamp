...
- **Reusable layout helpers**: `<PageHead>`, `<StatusPill>`, `<Modal>`, `<DataPair>`, `<KegiatanForm>` (penelitian/pengabdian list+CRUD), `<Sidebar role>`, `<Topbar>`.
- **Angka ringkasan — pilih satu, jangan bikin sendiri.** Tile KPI berdiri sendiri (punya kotak, border, aksen kiri) = `<StatCard>` dari `@/ds`. Pasangan label+nilai *di dalam* sebuah Card = `<DataPair>`. Keduanya tidak boleh saling dibungkus (Card bersarang). Sampai Juli 2026 ada **11 salinan** komponen `Kpi`/`Stat`/`MiniStat`/`Metric` yang ditulis ulang per halaman dengan ukuran & arti warna yang berbeda — `highlight` berarti oranye di satu halaman dan MERAH di halaman lain. Jangan ulangi: kalau butuh varian baru, tambah `tone` ke komponen bersama.
